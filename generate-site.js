 ...
 *    - KEY destek: "ep:1", "break:1", "special:snow", "special:special0"
...
function makeSummaryKey(ep, index) {
  ...
  if (ep.kind === "special") {
    if (ep.extraType) return `special:${ep.extraType}`; // snow, special0 vs
...
function makeRelUrl(season, ep, index) {
  ...
  if (ep.kind === "special") {
    ...
    if (ep.extraType === "special0") return `sezon/${season}/ozel/frozen-bond.html`;
...
function makeMetaForEpisode(season, ep, pageUrl, summaryText = "") {
  ...
  let title = "";
  if (isSnow) title = `Re:Zero Memory Snow – Türkçe İzle | rezeroizle.com`;
  else if (isBreak) title = `Re:Zero ${season}. Sezon ${ep.number}. Ara Bölüm Türkçe İzle | rezeroizle.com`;
  else if (isSpecial) title = `Re:Zero ${season}. Sezon Özel Bölüm Türkçe İzle | rezeroizle.com`;
  else title = `Re:Zero ${season}. Sezon ${ep.number}. Bölüm Türkçe Altyazılı İzle (Full HD) | rezeroizle.com`;
...
function makeMetaForHub(season, pageUrl) {
  const title = `Re:Zero ${season}. Sezon Bölümleri – Türkçe Altyazılı İzle | rezeroizle.com`;
...
function navLabelForEp(ep) {
  ...
  if (ep.kind === "special") {
    ...
    if (ep.extraType === "special0") return "Frozen Bond";
    return "Özel Bölüm";
  }
  return "İçerik";
}
...
`
  ...
  const prevBtnHtml = prevRel
    ? `<a class="ep-nav-btn" href="${escAttr(prevRel)}">${escAttr(prevText)}</a>`
    : `<span class="ep-nav-btn disabled" aria-disabled="true">←</span>`;

  const nextBtnHtml = nextRel
    ? `<a class="ep-nav-btn" href="${escAttr(nextRel)}">${escAttr(nextText)}</a>`
    : `<span class="ep-nav-btn disabled" aria-disabled="true">→</span>`;
